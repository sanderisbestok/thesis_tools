...
def is_absolute_text_format(file_path, num_blocks=[6, 5], blocks_abs_values=[4]):
    ...
    if not is_empty_file(file_path):
        return all_lines_have_blocks(file_path, num_blocks=num_blocks) and all_blocks_have_absolute_values(
        ...
        for line in f:
            ...
            for block in blocks_abs_values:
                if len(splitted) < block:
                    return False
                try:
                    if (float(splitted[block]) == int(float(splitted[block]))) or True:
                        passed = True
                except:
                    passed = False
            if passed is False:
